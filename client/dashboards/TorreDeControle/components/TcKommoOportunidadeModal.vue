...
const form = ref({
  ...
  dores: (props.oportunidades || [])
    .slice(0, 4)
    .map(o => `- ${o.descricao || o.titulo}`).join('\n') || getCF(1989932) || '',
        ...
        <section class="kop-section">
          ...
          <div class="kop-fields">
            ...
            <label class="kop-field">
              Tier
              <input :value="tier" type="text" disabled class="kop-readonly" />
              <span class="kop-field-hint">herdado do lead origem</span>
            </label>
            <label class="kop-field">
            ...
            <label class="kop-field">
              Squad
              <input v-model="form.squad" type="text" />
            </label>
            <label class="kop-field">
              Account
...
.kop-header h2 { font-size: 17px; color: #fff; margin: 0; font-weight: 600; }
.kop-sub {
  margin: 6px 0 0;
...
.kop-pipeline-chip {
  ...
  background: rgba(255,0,0,0.18); color: #ff6666;
  padding: 3px 10px; border-radius: 10px;
  letter-spacing: 0.3px; text-transform: uppercase;
}
.kop-sub-hint { font-size: 12px; color: #888; font-style: italic; }

.kop-close {
  background: transparent; border: none;
  color: #666; font-size: 26px; line-height: 1;
  cursor: pointer; padding: 0;
  transition: color 120ms;
}
.kop-close:hover { color: #fff; }
...
.kop-body::-webkit-scrollbar-thumb { background: #2a2a2a; border-radius: 3px; }
...
.kop-section-hint { font-size: 11.5px; color: #888; }
...
.kop-op:hover:not(.is-disabled) { border-color: rgba(255,255,255,0.1); }
.kop-op.is-selected {
  border-color: #ff0000;
...
.kop-op-check-icon { width: 12px; height: 12px; color: #fff; }
...
.kop-op-head strong { color: #fff; font-size: 13px; }
.kop-badge {
  font-size: 10.5px; font-weight: 700; padding: 2px 8px;
  border-radius: 9px; text-transform: uppercase; letter-spacing: 0.3px;
}
.badge--alta  { background: rgba(34,197,94,0.18);  color: #22c55e; }
.badge--media { background: rgba(245,158,11,0.18); color: #f59e0b; }
.badge--baixa { background: rgba(255,255,255,0.08); color: #aaa;   }

.kop-op-meta { font-size: 12px; color: #aaa; margin-top: 4px; }
...
.kop-op-form input:focus { border-color: #ff0000; }
...
.kop-field textarea:focus { border-color: #ff0000; }
...
.kop-total-label { font-size: 11.5px; color: #888; text-transform: uppercase; letter-spacing: 0.4px; }
.kop-total-val { font-size: 18px; color: #fff; font-weight: 700; }
...
.kop-btn--ghost:hover:not(:disabled) { background: #2a2a2a; color: #fff; }
.kop-btn--primary {
  background: #ff0000; color: #fff;
}
.kop-btn--primary:hover:not(:disabled) { background: #cc0000; }
